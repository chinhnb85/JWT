	...
	return func(res http.ResponseWriter, req *http.Request) {
		...
		case "password":
			...
			if err != nil {
				...
				http.Error(res, "Error occured", http.StatusInternalServerError)
				log.Fatal(err)
				return
			}
			if !userLogin.IsValidUser() {
				//response.MessageWithJson(res, "Username or Password invalid", http.StatusUnauthorized)
				http.Error(res, "Username or Password invalid", http.StatusUnauthorized)
				return
			} else {
				...
				if tokenResult.Error != nil {
					...
					http.Error(res, "Cant get token", http.StatusInternalServerError)
					return
				}
				//response.JsonResponse(res, tokenResult.AccessToken, http.StatusOK)
				http.Error(res, tokenResult.AccessToken.AccessToken, http.StatusOK)
			}

		default:
			//response.MessageWithJson(res, "Invalid grant type", http.StatusNotFound)
			http.Error(res, "Invalid grant type", http.StatusNotFound)
			return
		}
	}
}
